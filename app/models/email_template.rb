class EmailTemplate < ActiveRecord::Base
  ...
  def stats(email_activities)
    ...
    if delivery_count > 0
      ...
      detailed_stats = {total_processed: total_processed, open_count: open_count, open_percent: (open_count.to_f/delivery_count), delivery_count: delivery_count, delivery_percent: ((delivery_count.to_f/delivery_count)), clicked_count: clicked_count, clicked_percent: ((clicked_count.to_f/delivery_count)), bounce_count: bounce_count, bounce_percent: ((bounce_count.to_f/delivery_count))}
    else
