...
SET @sql = CONCAT('SELECT   a.name AS name,a.email as email,a.id as subscriber_id,a.subscriber_group_id as subscriber_group_id, ', @sql, '
                    FROM    subscribers a
                            LEFT JOIN custom_field_values b
                                ON a.id = b.subscriber_id where FIND_IN_SET(a.subscriber_group_id,"',group_ids,'")
                    GROUP   BY a.email');
...
BEGIN
select name,email,id as subscriber_id,subscriber_group_id from subscribers where FIND_IN_SET(subscriber_group_id,group_ids);
END$$
DELIMITER ;
